...
Pada arsitektur tersebut, ESP32-CAM berfungsi untuk melakukan akuisisi citra, pengumpulan telemetri, dan publikasi MQTT over WSS ke broker EMQX. Data gambar yang masuk kemudian diteruskan oleh backend Laravel ke layanan deteksi (detection service) terpisah di server Intel NUC. Di dalam layanan deteksi, citra dianalisis secara berurutan melalui algoritma deteksi gerakan (motion detection) dan deteksi penyusup (intruder detection). Jika terdeteksi adanya aktivitas atau penyusupan, status terupdate akan disiarkan ke dashboard website secara real-time via Laravel Reverb dan notifikasi dikirimkan ke aplikasi mobile.

\subsection{Perancangan Alur Komunikasi Data}

Proses komunikasi data dimulai dari akuisisi citra dan pengumpulan data telemetri oleh ESP32-CAM, yang kemudian dipublikasikan melalui protokol MQTT over WSS ke broker EMQX. Backend Laravel menerima data tersebut, menyimpan data telemetri ke MySQL dan berkas citra ke MinIO. Citra tersebut kemudian diteruskan ke layanan deteksi terisolasi di server untuk menjalankan deteksi gerakan (motion detection) dan deteksi penyusup (intruder detection). Hasil analisis kemudian dikirimkan secara real-time ke antarmuka website menggunakan Laravel Reverb, serta ke aplikasi mobile melalui layanan push notification.
...
Motion detection dirancang untuk berjalan pada layanan deteksi (detection service) di server Intel NUC. Alur ini menerima data citra hasil akuisisi ESP32-CAM yang dikirimkan melalui broker MQTT, lalu membandingkan frame citra yang baru masuk terhadap frame citra sebelumnya secara asinkron. Apabila perubahan piksel antar frame melebihi nilai ambang batas yang ditentukan, sistem menganggap terdapat aktivitas gerakan pada area pengawasan dan melanjutkan citra ke tahap deteksi penyusup (intruder detection).
...
Deteksi gerakan (motion detection) diimplementasikan di dalam layanan deteksi (detection service) terpisah pada server Intel NUC dengan membandingkan perbedaan nilai intensitas piksel antar frame citra hasil akuisisi ESP32-CAM yang diterima secara berurutan. Setelah gerakan terdeteksi, citra tersebut diproses lebih lanjut oleh modul deteksi penyusup (intruder detection) untuk mengidentifikasi adanya objek manusia secara real-time.
